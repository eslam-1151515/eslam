...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">تعديل قسم</h2>
     <?php $__env->endSlot(); ?>

    <div class="py-6">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <form method="POST" action="<?php echo e(route('categories.update', $category)); ?>" enctype="multipart/form-data" class="text-right" id="categoryEditForm">
                <?php echo csrf_field(); ?>
                <?php echo method_field('PUT'); ?>
                <div class="mb-4">
                    <label class="block mb-1 font-medium">اسم القسم <span class="text-red-500">*</span></label>
                    <input type="text" name="name_ar" value="<?php echo e(old('name_ar', $category->name_ar ?? $category->name)); ?>" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-200" required>
                        ...
                        <div class="flex-1 relative">
                            <select name="main_category" class="w-full border rounded-lg p-2 pr-8 focus:ring-2 focus:ring-indigo-200" required style="-webkit-appearance: none; -moz-appearance: none; appearance: none; background-image: none;">
                                ...
                                <?php $__currentLoopData = \App\Models\Category::getMainCategories(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mainCat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($mainCat); ?>" <?php echo e(old('main_category', $category->main_category) == $mainCat ? 'selected' : ''); ?>>
                                        <?php echo e($mainCat); ?>

                                    </option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                            <!-- Custom dropdown arrow pointing down on the left -->
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-200" rows="4"><?php echo e(old('description', $category->description)); ?></textarea>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="mb-4">
                    ...
                    <?php if($category->image_path): ?>
                        <div class="mb-2">
                            <img src="<?php echo e(asset('storage/'.$category->image_path)); ?>" alt="صورة القسم الحالية" class="h-24 w-24 object-cover rounded border">
                        </div>
                    <?php endif; ?>
                    <input type="file" name="image" accept="image/*" class="w-full border rounded-lg p-2" onchange="previewEditImage(event)">
                    <div class="mt-2">
                        <img id="imageEditPreview" class="hidden h-24 w-24 object-cover rounded border" alt="معاينة الصورة الجديدة">
                    </div>
...
unset($__errorArgs, $__bag); ?>
                ...
                <div class="mt-6 flex items-center justify-between">
                    ...
                    <button type="submit" id="submitEditBtn" class="inline-flex items-center gap-2 min-w-[160px] justify-center px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow focus:outline-none focus:ring-2 focus:ring-indigo-300">تحديث القسم</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
function previewEditImage(evt){
  const file = evt.target.files && evt.target.files[0];
  const img = document.getElementById('imageEditPreview');
...
<?php /**PATH E:\programing\flutter project\fast order\resources\views\categories\edit.blade.php ENDPATH**/ ?>