...
<head>
    ...
    <title>تسجيل تاجر جديد - <?php echo e(config('app.name', 'فاست أوردر')); ?></title>
...
<body class="min-h-screen flex flex-col justify-between py-8 px-4 sm:px-6 lg:px-8 relative">
    ...
    <header class="max-w-5xl mx-auto w-full flex items-center justify-between mb-8">
        ...
        <div class="flex items-center gap-4 text-sm">
            ...
            <a href="<?php echo e(Route::has('login') ? route('login') : url('/login')); ?>" class="px-5 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium transition duration-200 flex items-center gap-2">
    ...
    <main class="max-w-4xl mx-auto w-full flex-1 flex flex-col justify-center" x-data="merchantWizard()">
        <div class="glass-card rounded-3xl p-6 sm:p-10 relative overflow-hidden">
            ...
            <!-- Form -->
            <form id="wizardForm" method="POST" action="<?php echo e(Route::has('merchant.register.submit') ? route('merchant.register.submit') : (Route::has('register') ? route('register') : url('/register'))); ?>" @submit="handleSubmit($event)">
                <?php echo csrf_field(); ?>
                <?php if($googleUser): ?>
                        ...
                        <div class="p-5 rounded-2xl bg-brand-500/10 border border-brand-500/30 flex items-center justify-between mb-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-brand-300 text-xs"><?php echo e($googleUser['email'] ?? ''); ?></p>
                                </div>
                            </div>
                            <span class="px-3 py-1 rounded-full bg-brand-500/20 text-brand-300 text-xs font-semibold">موثق</span>
                        </div>
                        <input type="hidden" name="email" value="<?php echo e($googleUser['email'] ?? ''); ?>">
                    <?php else: ?>
                        <!-- Google OAuth Button -->
                        <div class="mb-6">
                            <a href="<?php echo e(Route::has('auth.google') ? route('auth.google') : url('/auth/google')); ?>"
                ...
                <div x-show="currentStep === 4" x-transition:enter="step-transition" x-cloak>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <?php $__empty_1 = true; $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div @click="form.plan_id = '<?php echo e($plan->id); ?>'; form.plan_name = '<?php echo e($plan->name); ?>'"
                                 class="cursor-pointer rounded-2xl p-5 border transition-all duration-300 relative flex flex-col justify-between"
                                 :class="form.plan_id == '<?php echo e($plan->id); ?>' ? 'bg-gradient-to-b from-brand-900/40 to-black border-brand-400 shadow-glow scale-[1.02]' : 'bg-black/40 border-white/10 hover:border-white/20'">
                                
                                <?php if($loop->first || $plan->slug === 'pro'): ?>
                                ...
                                <div>
                                    <div class="flex justify-between items-start mb-3">
                                        <h3 class="font-bold text-lg text-white"><?php echo e($plan->name); ?></h3>
                                        <div class="w-5 h-5 rounded-full border flex items-center justify-center transition"
                                             :class="form.plan_id == '<?php echo e($plan->id); ?>' ? 'border-brand-400 bg-brand-400 text-black font-black text-xs' : 'border-gray-600'">
                                            <template x-if="form.plan_id == '<?php echo e($plan->id); ?>'">✓</template>
                                        </div>
                                    </div>
                                    <p class="text-xs text-gray-400 mb-4"><?php echo e($plan->description ?? 'باقة ممتازة لإدارة متجرك بكفاءة وسرعة.'); ?></p>
                                    <div class="mb-5">
                                        <span class="text-2xl font-black text-white"><?php echo e(number_format($plan->price_monthly ?? 0)); ?></span>
                                    ...
                                    <ul class="space-y-2.5 text-xs text-gray-300 border-t border-white/5 pt-4">
                                        <li class="flex items-center gap-2">
                                            ...
                                            <span><strong><?php echo e($plan->trial_days ?? 14); ?> يوماً</strong> تجربة مجانية</span>
                                        </li>
                                        <li class="flex items-center gap-2">
                                            <span class="text-brand-400 font-bold">✓</span>
                                ...
                                <div class="mt-6 pt-3 text-center border-t border-white/5">
                                    <span class="text-[11px] font-semibold" :class="form.plan_id == '<?php echo e($plan->id); ?>' ? 'text-brand-300' : 'text-gray-500'">اختر هذه الباقة</span>
                                </div>
                            </div>
                    ...
                    <div class="bg-gradient-to-b from-brand-900/30 via-black/40 to-black/60 rounded-2xl p-6 border border-brand-500/30 text-center mb-6 relative overflow-hidden">
                        ...
                        <div class="bg-black/50 rounded-xl p-4 text-right space-y-3 border border-white/10 text-xs sm:text-sm max-w-lg mx-auto">
                            ...
                            <div class="flex justify-between items-center py-1.5 border-b border-white/5">
                                ...
                                <span class="font-semibold text-white" x-text="form.email || '<?php echo e($googleUser['email'] ?? '---'); ?>'"></span>
                            </div>
                            <div class="flex justify-between items-center py-1.5 border-b border-white/5">
    ...
    <footer class="max-w-4xl mx-auto w-full text-center mt-8 text-xs text-gray-500">
        <p>© <?php echo e(date('Y')); ?> فاست أوردر (Fast Order). جميع الحقوق محفوظة. بتسجيلك أنت توافق على <a href="#" class="text-gray-400 underline">شروط الخدمة</a> و <a href="#" class="text-gray-400 underline">سياسة الخصوصية</a>.</p>
    </footer>
    ...
    <script>
        function merchantWizard() {
            return {
                ...
                form: {
                    email: '<?php echo e(old('email', $googleUser['email'] ?? '')); ?>',
                    password: '',
                    password_confirmation: '',
                    name: '<?php echo e(old('name', $googleUser['name'] ?? '')); ?>',
                    phone: '<?php echo e(old('phone')); ?>',
                    store_name: '<?php echo e(old('store_name')); ?>',
                    slug: '<?php echo e(old('slug')); ?>',
                    activity: '<?php echo e(old('activity', 'ملابس وأزياء')); ?>',
                    plan_id: '<?php echo e(old('plan_id', '')); ?>',
                    plan_name: ''
                },
                init() {
                    <?php if($googleUser): ?>
                    ...
                    <?php if($errors->any()): ?>
                        ...
                        <?php if($errors->has('store_name') || $errors->has('slug')): ?>
                            this.currentStep = 3;
                ...
                async checkSlugAvailability() {
                    ...
                    try {
                        const response = await fetch('<?php echo e(url('/register/check-slug')); ?>?slug=' + encodeURIComponent(this.form.slug));
...
<?php /**PATH E:\programing\flutter project\fast order\resources\views\platform\auth\register.blade.php ENDPATH**/ ?>