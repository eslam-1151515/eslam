...
<head>
    ...
    <title>????? - <?php echo e($tenant->name); ?></title>
...
<body class="theme-starter fast-theme">
    ...
    <div class="top-bar">
        <a href="/shop/">? ?????? ??????</a>
        <div class="store-brand">??? <span><?php echo e($tenant->name); ?></span></div>
        <a href="/shop/cart.html" style="background: rgba(108,99,255,0.1); padding: 0.5rem 1.2rem; border-radius: 20px;">?? ?????</a>
    </div>
            ...
            <main class="content-card">
                ...
                <div class="tab-content active" id="tab-orders">
                    ...
                    <div class="filter-pills">
                        <button class="filter-pill active" onclick="filterOrders('all', this)">????</button>
                        <button class="filter-pill" onclick="filterOrders('pending', this)">? ??? ????????</button>
                        <button class="filter-pill" onclick="filterOrders('confirmed', this)">?? ????</button>
                        <button class="filter-pill" onclick="filterOrders('processing', this)">?? ???? ???????</button>
                        <button class="filter-pill" onclick="filterOrders('shipped', this)">?? ?? ?????</button>
                        <button class="filter-pill" onclick="filterOrders('delivered', this)">? ?? ???????</button>
                        <button class="filter-pill" onclick="filterOrders('cancelled', this)">? ????</button>
                    </div>

                    <div id="orders-list">
                        <div class="empty-state"><div class="icon">?</div><p>???? ????? ??????...</p></div>
                    </div>
                ...
                <div class="tab-content" id="tab-addresses">
                    <div class="tab-header">
                        <h2 class="tab-title">?? ?????? ????? ????????</h2>
                        <button type="button" class="btn-primary" style="padding:0.6rem 1.25rem; font-size:0.9rem;" onclick="openAddressModal()">? ????? ????? ????</button>
                    </div>
                    <div id="addresses-list" class="addresses-grid">
                        <div class="empty-state" style="grid-column: 1/-1;"><div class="icon">?</div><p>???? ????? ????????...</p></div>
                    </div>
                </div>
                    ...
                    <form onsubmit="updateProfile(event)" id="profile-form">
                        <div class="form-grid">
                            <div class="form-group full-width">
                                ...
                                <input type="text" id="profile-name" class="form-input" required placeholder="???? ???? ??????">
                            </div>
                        ...
                        <div style="margin-top: 1rem;">
                            <button type="submit" class="btn-primary" id="save-profile-btn">?? ??? ?????????</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="tab-content" id="tab-password">
                    ...
                    <form onsubmit="updatePassword(event)" style="max-width: 480px;">
                        ...
                        <div class="form-group">
                            <label class="form-label">???? ?????? ???????</label>
                            <input type="password" id="new-password" class="form-input" required minlength="8" placeholder="8 ???? ??? ?????">
                        </div>
                        <div class="form-group">
                            <label class="form-label">????? ???? ?????? ???????</label>
                ...
                <div class="tab-content" id="tab-account-settings">
                    <div class="tab-header">
                        <h2 class="tab-title" style="color: #e11d48;">?? ????? ???? ??????</h2>
                    </div>
                    <p style="color: var(--text-muted); font-size: 0.95rem;">????? ?? ??? ????? ???? ????? ?? ???? <?php echo e($tenant->name); ?>. ???? ???????? ?? ??? ????????? ?? ???? ??????? ????.</p>
                    ...
                    <div class="danger-zone">
                        ...
                        <button type="button" class="btn-danger" onclick="openDeleteModal()">??? ??? ????? ???????</button>
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="order-modal">
        <div class="modal-box">
            <div class="modal-header">
                <h3 class="modal-title" id="modal-order-title">#XXXXX ?????? ????? ?????</h3>
    ...
    <div class="modal-overlay" id="address-modal">
        <div class="modal-box" style="max-width: 520px;">
            <div class="modal-header">
                <h3 class="modal-title" id="modal-address-title">? ????? ????? ??? ????</h3>
            ...
            <form onsubmit="saveAddress(event)">
                ...
                <div class="form-group">
                    <label class="form-label">????? ???? (????: ??????? ?????)</label>
                    <input type="text" id="addr-title" class="form-input" required placeholder="??????">
                </div>
                <div class="form-grid">
                    <div class="form-group">
                        <label class="form-label">???????? / ???????</label>
                ...
                <div class="form-group">
                    <label class="form-label">??????? ???????? (??????? ??? ??????? ?????)</label>
                    <textarea id="addr-text" class="form-input" required rows="3" placeholder="???? ??????? ???????? ????? ???? ???????..."></textarea>
                </div>
                <div class="form-group" style="display:flex; align-items:center; gap:0.5rem;">
                    <input type="checkbox" id="addr-default" style="width:18px; height:18px;">
                    <label for="addr-default" style="font-weight:700; font-size:0.9rem; cursor:pointer;">? ????? ?????? ????? ?????????</label>
                </div>
                <div style="margin-top: 1.5rem; text-align: left;">
                    <button type="submit" class="btn-primary" style="width:100%; justify-content:center;">?? ??? ???????</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="delete-modal">
        <div class="modal-box" style="max-width: 480px; text-align: center;">
            ...
            <h3 style="font-size: 1.35rem; font-weight: 900; color: #9f1239; margin-bottom: 0.75rem;">?? ??? ????? ?? ??? ??????</h3>
            <p style="color: #64748b; font-size: 0.95rem; margin-bottom: 1.5rem; line-height: 1.6;">??? ??????? ????? ??? ???? ??????? ???. ???? ????? ???? ?????? ??????? ?????? ??????.</p>
            ...
            <form onsubmit="confirmDeleteAccount(event)">
                ...
                <div style="display: flex; gap: 0.75rem; margin-top: 1.5rem;">
                    <button type="button" class="btn-outline" style="flex:1;" onclick="closeModal('delete-modal')">?????</button>
                    <button type="submit" class="btn-danger" style="flex:1;">???? ???? ?????</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function loadProfile() {
            fetch('/api/account/profile')
                ...
                .then(function(data) {
                    ...
                    document.getElementById('user-since').textContent = '??? ??? ' + (u.created_at || '');
            ...
            fetch('/api/account/orders?page=' + page)
                ...
                .then(function(data) {
                    ...
                    if (!data.orders || data.orders.length === 0) {
                        if (page === 1) {
                            container.innerHTML = '<div class="empty-state"><div class="icon">??</div><h3>?? ???? ????? ???</h3><p>?? ??? ?????? ?? ??? ?? ?????? ??? ????.</p><a href="/shop/" class="btn-primary">??? ???? ????????</a></div>';
                        }
                    ...
                    if (data.has_more) {
                        var nextPage = page + 1;
                        loadMoreEl.innerHTML = '<button class="btn-outline" onclick="loadOrders(' + nextPage + ')">? ????? ?????? ?? ???????</button>';
                    } else {
                        loadMoreEl.innerHTML = '';
                    }
                })
                .catch(function() {
                    document.getElementById('orders-list').innerHTML = '<div class="empty-state"><div class="icon">??</div><h3>??? ??? ????? ????? ???????</h3><p>???? ????? ???????? ??????</p></div>';
                });
        }
        ...
        function renderOrdersList() {
            ...
            if (filtered.length === 0) {
                container.innerHTML = '<div class="empty-state" style="padding:2rem;"><div class="icon" style="font-size:2.5rem;">??</div><p>?? ???? ????? ????? ??? ???????</p></div>';
                return;
            }
            ...
            container.innerHTML = filtered.map(function(o) {
                ...
                if (o.items && o.items.length > 0) {
                    ...
                    if (o.items.length > 3) {
                        itemsPreview += '<span style="color:var(--text-muted); font-size:0.8rem; font-weight:700;">+' + (o.items.length - 3) + ' ????</span>';
                    }
                } else {
                    itemsPreview = '<span>' + o.items_count + ' ??????</span>';
                }
                ...
                return '<div class="order-card">' +
                    ...
                    '<div class="order-body">' +
                        ...
                        '<div class="order-total-box">' +
                            '<div class="order-total-lbl">????????:</div>' +
                            '<div class="order-total-val">' + o.total + ' ?.?</div>' +
                        '</div>' +
                    '</div>' +
                    '<div class="order-footer">' +
                        '<span style="font-size:0.85rem; color:#64748b; font-weight:700;">' + (o.payment_method_label || '?? ????? ??? ????????') + '</span>' +
                        '<div style="display:flex; gap:0.5rem;">' +
                            '<button type="button" class="btn-track" onclick="openOrderModal(' + o.id + ')">??? ???? ?????? ??????? ??????</button>' +
                            '<a href="/track-order?order=' + o.order_number + '" class="btn-track" style="text-decoration:none; color:var(--primary); border-color:var(--primary);">?? ???? ?????</a>' +
                        '</div>' +
                    '</div>' +
                '</div>';
            }).join('');
        ...
        function openOrderModal(orderId) {
            ...
            document.getElementById('modal-order-title').textContent = '#' + o.order_number + ' ?????? ????? ?????';
            ...
            // Items HTML
            var itemsHtml = '<div class="order-modal-section"><h4 class="section-title">??? ?????? ????? (' + o.items_count + ')</h4>';
            if (o.items && o.items.length > 0) {
                itemsHtml += o.items.map(function(it) {
                    var img = it.image || '/shop/placeholder.jpg';
                    var meta = [];
                    if (it.selectedSize) meta.push('??????: ' + it.selectedSize);
                    if (it.selectedColor) meta.push('?????: ' + it.selectedColor);
                    return '<div class="modal-item-row">' +
                        ...
                        '<div style="text-align:left;">' +
                            '<div class="modal-item-price">' + parseFloat(it.price || 0).toFixed(0) + ' ?.?</div>' +
                            '<div style="font-size:0.8rem; color:#64748b; font-weight:700;">??????: ' + (it.quantity || it.qty || 1) + '</div>' +
                        '</div>' +
                    '</div>';
                }).join('');
            } else {
                itemsHtml += '<p style="color:var(--text-muted); font-size:0.9rem;">?????? ???????? ??????? ??? ????? ?? ??? ?????</p>';
            }
            itemsHtml += '</div>';

            // Shipping & Payment Summary
            var summaryHtml = '<div class="order-modal-section"><h4 class="section-title">?? ?????? ??????? ??????</h4>' +
                '<div style="background:#f8fafc; padding:1.25rem; border-radius:16px; font-size:0.9rem; line-height:1.8;">' +
                    ...
                    '<p><strong>??? ??????:</strong> ' + (o.customer_phone || '??? ????') + '</p>' +
                    '<p><strong>???????:</strong> ' + (o.shipping_address || '') + ' (' + (o.governorate || '') + ')</p>' +
                    '<p><strong>????? ?????:</strong> ' + (o.payment_method_label || '????? ??? ????????') + '</p>' +
                    '<div style="height:1px; background:#e2e8f0; margin:0.75rem 0;"></div>' +
                    '<div style="display:flex; justify-content:space-between;"><span>??????? ??????:</span><span>' + o.subtotal + ' ?.?</span></div>' +
                    '<div style="display:flex; justify-content:space-between;"><span>???? ?????:</span><span>' + o.shipping_cost + ' ?.?</span></div>' +
                    '<div style="display:flex; justify-content:space-between; font-weight:900; font-size:1.1rem; color:var(--primary); margin-top:0.5rem;"><span>???????? ?????:</span><span>' + o.total + ' ?.?</span></div>' +
                '</div></div>';
        ...
        function renderAddresses() {
            ...
            if (!userAddresses || userAddresses.length === 0) {
                container.innerHTML = '<div class="empty-state" style="grid-column: 1/-1;"><div class="icon">??</div><h3>?? ???? ?????? ??? ??????</h3><p>??? ????? ??? ???? ?????? ????? ?????? ??????? ??? ?????.</p><button type="button" class="btn-primary" onclick="openAddressModal()">? ????? ????? ????</button></div>';
                return;
            }
                ...
                return '<div class="address-card ' + (a.is_default ? 'default' : '') + '">' +
                    '<div>' +
                        '<div class="address-header">' +
                            '<span class="address-title">?? ' + (a.title || '????? ???') + '</span>' +
                            (a.is_default ? '<span class="default-tag">? ?????????</span>' : '') +
                        '</div>' +
                        '<p class="address-text">' + (a.address || '') + ' · <strong>' + (a.governorate || '') + '</strong></p>' +
                        '<p class="address-phone">?? ' + (a.phone || '') + '</p>' +
                    '</div>' +
                    '<div class="address-actions">' +
                        '<button type="button" class="addr-btn" onclick="editAddress(\'' + a.id + '\')">?? ?????</button>' +
                        (!a.is_default ? '<button type="button" class="addr-btn" onclick="setDefaultAddress(\'' + a.id + '\')">? ???????</button>' : '') +
                        '<button type="button" class="addr-btn del" onclick="deleteAddress(\'' + a.id + '\')">??? ???</button>' +
                    '</div>' +
                '</div>';
            }).join('');
        }
        ...
        function openAddressModal(addrId) {
            ...
            if (availableGovs && availableGovs.length > 0) {
                select.innerHTML = '<option value="">???? ???????? / ???????</option>' + availableGovs.map(function(g) {
                    return '<option value="' + g.name + '">' + g.name + ' (' + g.price + ' ?.?)</option>';
                }).join('');
            } else {
                select.innerHTML = '<option value="???????">???????</option><option value="??????">??????</option><option value="??????????">??????????</option><option value="????">????</option>';
            }

            if (addrId) {
                var a = userAddresses.find(function(item) { return item.id === addrId; });
                if (a) {
                    document.getElementById('modal-address-title').textContent = '?? ????? ????? ?????';
        ...
        function loadAccountWishlist() {
            fetch('/api/wishlist')
                    ...
                    container.innerHTML = '<div class="wishlist-grid">' + data.items.map(function(item) {
                        return '<div class="wishlist-card">' +
                            '<div class="wishlist-img-box">' +
                                ...
                                '<button type="button" class="wishlist-del-btn" title="????? ?? ???????" onclick="removeWishitem(' + item.product_id + ', this)">???</button>' +
                            '</div>' +
                            '<div class="wishlist-info">' +
                                '<div>' +
                                    '<div class="wishlist-name">' + item.name + '</div>' +
                                    '<div class="wishlist-price">' + parseFloat(item.price).toFixed(0) + ' ?.?</div>' +
                                '</div>' +
                                '<a href="/shop/product.html?id=' + item.product_id + '" class="wishlist-link">?? ??? ?????? ???????</a>' +
                            '</div>' +
                        '</div>';
                    }).join('') + '</div>';
                ...
                .catch(function() {
                    document.getElementById('account-wishlist').innerHTML = '<div class="empty-state"><div class="icon">??</div><h3>??? ??? ????? ????? ???????</h3></div>';
                });
        }
        ...
        function updatePassword(e) {
            ...
            if (newPass !== confirmPass) { showToast('????? ?????? ??? ?????????', false); return; }
...
<?php /**PATH E:\programing\flutter project\fast order\resources\views\shop\themes\starter\account.blade.php ENDPATH**/ ?>