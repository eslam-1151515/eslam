<?php $__env->startSection('title', 'معاينة وتخصيص الثيم - ' . ($themeConfig['name'] ?? $theme)); ?>
...
<div class="min-h-screen bg-gray-900 text-gray-100 flex flex-col font-['Almarai']" dir="rtl">
    ...
    <header class="bg-gray-800 border-b border-gray-700 px-6 py-3 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-50">
        <div class="flex items-center gap-3">
            <a href="<?php echo e(url('/admin/themes')); ?>" class="bg-gray-700 hover:bg-gray-600 text-gray-200 px-3 py-1.5 rounded-lg text-sm transition flex items-center gap-2">
                <span>&larr;</span> خروج من المعاينة
            </a>
            <h1 class="text-lg font-bold text-white flex items-center gap-2">
                <span>🎨 معاينة الثيم:</span>
                <span class="text-emerald-400"><?php echo e($themeConfig['name'] ?? $theme); ?></span>
                <?php if($activeThemeSlug === $theme): ?>
        ...
        <div class="flex items-center bg-gray-900/80 p-1 rounded-xl border border-gray-700 gap-1" id="device-switcher">
            <?php $__currentLoopData = $deviceModes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $mode): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <button type="button" 
                        onclick="switchDevice('<?php echo e($key); ?>', '<?php echo e($mode['width']); ?>', '<?php echo e($mode['height']); ?>', '<?php echo e($mode['max_width']); ?>')"
                        id="btn-device-<?php echo e($key); ?>"
                        class="px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition <?php echo e($key === 'desktop' ? 'bg-indigo-600 text-white shadow-md' : 'text-gray-400 hover:text-white hover:bg-gray-800'); ?>">
                    <?php if($key === 'desktop'): ?>
                        <span>💻</span>
                    <?php elseif($key === 'tablet'): ?>
                        <span>📱</span>
                    <?php else: ?>
                        <span>📲</span>
                    <?php endif; ?>
        ...
        <aside class="w-80 bg-gray-800 border-l border-gray-700 overflow-y-auto p-5 space-y-6 shrink-0">
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-1 gap-2.5 max-h-48 overflow-y-auto pr-1">
                    <?php $__currentLoopData = $allThemes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $themeSlug => $tConfig): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(route('merchant.themes.preview.index', ['slug' => $themeSlug])); ?>" 
                           class="flex items-center gap-3 p-2 rounded-xl border transition <?php echo e($themeSlug === $theme ? 'bg-indigo-900/40 border-indigo-500 text-white' : 'bg-gray-900/60 border-gray-700 text-gray-300 hover:border-gray-600'); ?>">
                            <div class="w-12 h-10 rounded-lg overflow-hidden shrink-0 bg-gray-800 border border-gray-700 flex items-center justify-center">
                                <?php if(isset($thumbnails[$themeSlug]['svg_thumbnail'])): ?>
                                    <img src="<?php echo e($thumbnails[$themeSlug]['svg_thumbnail']); ?>" alt="<?php echo e($tConfig['name']); ?>" class="w-full h-full object-cover">
                                <?php else: ?>
                                    <span class="text-lg">🎨</span>
                                <?php endif; ?>
                            </div>
                            <div class="overflow-hidden">
                                <h4 class="text-xs font-bold truncate"><?php echo e($tConfig['name']); ?></h4>
                                <span class="text-[10px] text-gray-400"><?php echo e($tConfig['author'] ?? 'Fast Order'); ?></span>
                            </div>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            ...
            <form id="theme-customizer-form" class="space-y-5" onsubmit="return false;">
                    ...
                    <div class="bg-gray-900/80 p-3 rounded-xl border border-gray-700 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" id="input-primary-color" name="primary_color" 
                                   value="<?php echo e($customizations['primary_color'] ?? '#4f46e5'); ?>" 
                                   oninput="updateLivePreview('primary_color', this.value)"
                                   class="w-9 h-9 rounded-lg border-0 cursor-pointer bg-transparent">
                            <span id="hex-primary-color" class="text-xs font-mono text-gray-400"><?php echo e($customizations['primary_color'] ?? '#4f46e5'); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-900/80 p-3 rounded-xl border border-gray-700 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" id="input-secondary-color" name="secondary_color" 
                                   value="<?php echo e($customizations['secondary_color'] ?? '#64748b'); ?>" 
                                   oninput="updateLivePreview('secondary_color', this.value)"
                                   class="w-9 h-9 rounded-lg border-0 cursor-pointer bg-transparent">
                            <span id="hex-secondary-color" class="text-xs font-mono text-gray-400"><?php echo e($customizations['secondary_color'] ?? '#64748b'); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-900/80 p-3 rounded-xl border border-gray-700 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" id="input-background-color" name="background_color" 
                                   value="<?php echo e($customizations['background_color'] ?? '#ffffff'); ?>" 
                                   oninput="updateLivePreview('background_color', this.value)"
                                   class="w-9 h-9 rounded-lg border-0 cursor-pointer bg-transparent">
                            <span id="hex-background-color" class="text-xs font-mono text-gray-400"><?php echo e($customizations['background_color'] ?? '#ffffff'); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-900/80 p-3 rounded-xl border border-gray-700 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" id="input-text-color" name="text_color" 
                                   value="<?php echo e($customizations['text_color'] ?? '#1e293b'); ?>" 
                                   oninput="updateLivePreview('text_color', this.value)"
                                   class="w-9 h-9 rounded-lg border-0 cursor-pointer bg-transparent">
                            <span id="hex-text-color" class="text-xs font-mono text-gray-400"><?php echo e($customizations['text_color'] ?? '#1e293b'); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <select id="input-font-family" name="font_family" onchange="updateLivePreview('font_family', this.value)" class="w-full bg-gray-900 border border-gray-700 rounded-xl px-3 py-2 text-sm text-gray-200 focus:ring-2 focus:ring-indigo-500">
                        <option value="Cairo" <?php echo e(($customizations['font_family'] ?? '') === 'Cairo' ? 'selected' : ''); ?>>Cairo (خط كايرو العصري)</option>
                        <option value="Tajawal" <?php echo e(($customizations['font_family'] ?? '') === 'Tajawal' ? 'selected' : ''); ?>>Tajawal (خط تجول الأنيق)</option>
                        <option value="Almarai" <?php echo e(($customizations['font_family'] ?? '') === 'Almarai' ? 'selected' : ''); ?>>Almarai (خط المراعي المتوازن)</option>
                        <option value="Inter" <?php echo e(($customizations['font_family'] ?? '') === 'Inter' ? 'selected' : ''); ?>>Inter (تصميم تقني حديث)</option>
                        <option value="Roboto" <?php echo e(($customizations['font_family'] ?? '') === 'Roboto' ? 'selected' : ''); ?>>Roboto (كلاسيكي عالمي)</option>
                    </select>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            ...
                            <select id="input-header-layout" name="header_layout" onchange="updateLivePreview('header_layout', this.value)" class="w-full bg-gray-900 border border-gray-700 rounded-lg text-xs text-gray-200 p-1.5">
                                <option value="Classic" <?php echo e(($customizations['header_layout'] ?? '') === 'Classic' ? 'selected' : ''); ?>>كلاسيك</option>
                                <option value="Centered" <?php echo e(($customizations['header_layout'] ?? '') === 'Centered' ? 'selected' : ''); ?>>متمركز</option>
                                <option value="Minimal" <?php echo e(($customizations['header_layout'] ?? '') === 'Minimal' ? 'selected' : ''); ?>>بسيط</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="input-banner-layout" name="banner_layout" onchange="updateLivePreview('banner_layout', this.value)" class="w-full bg-gray-900 border border-gray-700 rounded-lg text-xs text-gray-200 p-1.5">
                                <option value="Slider" <?php echo e(($customizations['banner_layout'] ?? '') === 'Slider' ? 'selected' : ''); ?>>متحرك (Slider)</option>
                                <option value="Grid" <?php echo e(($customizations['banner_layout'] ?? '') === 'Grid' ? 'selected' : ''); ?>>شبكي (Grid)</option>
                                <option value="Single" <?php echo e(($customizations['banner_layout'] ?? '') === 'Single' ? 'selected' : ''); ?>>ثابت</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-indigo-950/40 border border-indigo-800/60 rounded-xl p-3 text-xs space-y-1">
                    ...
                    <div class="text-gray-300">الاسم: <span class="font-semibold text-white"><?php echo e($storeData['store_name']); ?></span></div>
                    <div class="text-gray-300">الأقسام: <span class="font-semibold text-white"><?php echo e(count($storeData['categories'])); ?></span> قسم الرئيسي</div>
                    <div class="text-gray-300">المنتجات المعروضة: <span class="font-semibold text-white"><?php echo e(count($storeData['products'])); ?></span> منتج</div>
                </div>
            </form>
        </aside>
        ...
        <main class="flex-1 bg-gray-950 flex items-center justify-center p-6 overflow-auto relative">
            <div id="viewport-container" class="transition-all duration-500 ease-in-out bg-white rounded-2xl overflow-hidden shadow-2xl border-4 border-gray-800 relative flex flex-col w-full h-full max-w-[1440px]">
                ...
                <iframe id="preview-iframe" 
                        src="<?php echo e($previewFrameUrl); ?>" 
...
<script>
    let currentThemeSlug = "<?php echo e($theme); ?>";
    let liveCustomizations = <?php echo json_encode($customizations, 15, 512) ?>;
    ...
    function changePreviewPage(page) {
        ...
        const baseUrl = "<?php echo e(url('/merchant/themes/preview/' . $theme . '/frame')); ?>";
    ...
    function saveAndActivateTheme() {
        ...
        .then(data => {
            if (data.status === 'success') {
                alert(data.message);
                window.location.href = "<?php echo e(url('/admin/themes')); ?>";
            } else {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\programing\flutter project\fast order\resources\views\merchant\themes\preview.blade.php ENDPATH**/ ?>