...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">المنتجات</h2>
     <?php $__env->endSlot(); ?>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col gap-3 mb-4">
            <div class="flex items-center justify-between gap-2">
                <a href="<?php echo e(route('products.create')); ?>" class="px-4 py-2 bg-white text-indigo-700 hover:text-white hover:bg-indigo-700 border border-indigo-700 rounded-lg shadow-sm">+ إضافة منتج</a>
                <form method="GET" class="flex items-center gap-2" action="<?php echo e(route('products.index')); ?>">
                    <input type="text" name="q" value="<?php echo e($q ?? ''); ?>" class="border rounded-lg p-2 w-64" placeholder="ابحث بالاسم أو الوصف أو القسم...">
            ...
            <h2 class="text-xl font-semibold text-right">قائمة المنتجات</h2>
        </div>
            ...
            <table class="w-full text-center text-sm align-middle table-fixed">
                ...
                <thead>
                    <tr class="border-b bg-gray-50">
                        ...
                        <th class="py-3 px-3 text-center">القسم</th>
                        <th class="py-3 px-3 text-center">السعر</th>
                        <th class="py-3 px-3 text-center">الكمية</th>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="border-b even:bg-gray-50">
                        <td class="py-3 px-3 align-middle text-center"><?php echo e(($products->currentPage()-1)*$products->perPage() + $i + 1); ?></td>
                        <td class="py-3 px-3 align-middle text-center">
                            <?php if($product->main_image_path): ?>
                                <img src="<?php echo e(asset('storage/'.$product->main_image_path)); ?>" alt="<?php echo e($product->name); ?>" class="h-12 w-12 object-cover rounded-md border mx-auto">
                            <?php else: ?>
                                <div class="h-12 w-12 mx-auto flex items-center justify-center rounded-md border text-gray-400 bg-gray-50">لا صورة</div>
                            <?php endif; ?>
                        </td>
                        <td class="py-3 px-3 align-middle text-center"><?php echo e($product->name); ?></td>
                        <td class="py-3 px-3 align-middle text-center"><?php echo e(optional($product->category)->name_ar ?? optional($product->category)->name); ?></td>
                        <td class="py-3 px-3 align-middle text-center"><?php echo e(number_format($product->price, 0)); ?></td>
                        <td class="py-3 px-3 align-middle text-center"><?php echo e($product->stock); ?></td>
                        <td class="py-2 px-3 align-middle">
                            <div class="flex items-center gap-2 justify-center">
                                <a href="<?php echo e(route('products.edit', $product)); ?>" class="inline-flex items-center justify-center h-9 w-9 rounded-md border hover:bg-blue-50 text-blue-600" title="تعديل" aria-label="تعديل">
                                ...
                                <form action="<?php echo e(route('products.destroy', $product)); ?>" method="POST" onsubmit="return confirmDelete(event)" class="inline">
                                    <?php echo csrf_field(); ?>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr><td colspan="5" class="py-6 text-center text-gray-500">لا توجد منتجات بعد</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
            </div>
...
function confirmDelete(e){
  if(!confirm('هل تريد حذف هذا المنتج؟')){ e.preventDefault(); return false; }
  return true;
}
</script>
<?php /**PATH E:\programing\flutter project\fast order\resources\views\products\index.blade.php ENDPATH**/ ?>