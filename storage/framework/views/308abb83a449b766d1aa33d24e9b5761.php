...
<body class="min-h-screen flex flex-col bg-slate-950 text-slate-100 selection:bg-indigo-500 selection:text-white" x-data="{ billing: 'yearly', activeFaq: null }">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <?php if(auth()->guard()->check()): ?>
                    <a href="<?php echo e(url('/dashboard')); ?>" class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm shadow-lg shadow-indigo-500/25 transition-all transform hover:-translate-y-0.5">
                        <i class="fa-solid fa-gauge-high ml-1.5"></i> لوحة التحكم
                    </a>
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="px-4 py-2 rounded-xl text-slate-300 hover:text-white hover:bg-white/5 font-semibold text-sm transition-all">
                        تسجيل الدخول
                    </a>
                    <?php if(Route::has('register')): ?>
                        <a href="<?php echo e(route('register')); ?>" class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-pink-600 hover:from-indigo-500 hover:to-pink-500 text-white font-bold text-sm shadow-lg shadow-indigo-500/25 transition-all transform hover:-translate-y-0.5">
    ...
    <section id="pricing-cards" class="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10 w-full">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-8 items-stretch">
            <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="relative flex flex-col rounded-3xl transition-all duration-300 hover:-translate-y-2 <?php echo e($isPopular ? 'glass-card-popular md:-translate-y-4 md:scale-105 z-20' : 'glass-card hover:border-slate-700 z-10'); ?> p-8">
                    <?php if($isPopular): ?>
                    ...
                    <div class="mb-6">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="text-2xl font-black text-white"><?php echo e($name); ?></h3>
                            <?php if($isPopular): ?>
                                <span class="w-3 h-3 rounded-full bg-emerald-400 animate-ping"></span>
                            <?php endif; ?>
                        </div>
                        <p class="text-sm text-slate-400 min-h-[40px] leading-relaxed"><?php echo e($description); ?></p>
                    </div>
                    ...
                    <div class="my-6 p-6 rounded-2xl bg-slate-900/60 border border-slate-800/80 text-center">
                        <div class="flex items-baseline justify-center gap-1.5">
                            <span class="text-4xl sm:text-5xl font-black text-white tracking-tight" 
                                  x-text="billing === 'monthly' ? '<?php echo e(number_format($priceMonthly, 0)); ?>' : '<?php echo e(number_format(round($priceYearly / 12), 0)); ?>'">
                                <?php echo e(number_format($priceMonthly, 0)); ?>
                        ...
                        <div class="text-xs text-indigo-400 mt-2 font-bold flex items-center justify-center gap-1">
                            ...
                            <span x-show="billing === 'yearly'" x-cloak>توفير سنوي: تدفع <?php echo e(number_format($priceYearly, 0)); ?> ر.س مرة واحدة سنوياً</span>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-6">
                        <div class="p-3 rounded-xl bg-white/5 border border-white/5 text-center">
                            ...
                            <span class="font-bold text-white text-sm">
                                <?php echo e($maxProducts >= 9999 ? 'غير محدود 🚀' : number_format($maxProducts) . ' منتج'); ?>
                        ...
                        <div class="p-3 rounded-xl bg-white/5 border border-white/5 text-center">
                            ...
                            <span class="font-bold text-white text-sm">
                                <?php echo e($maxOrders >= 9999 ? 'غير محدود 🚀' : number_format($maxOrders) . ' طلب'); ?>
                        ...
                        <ul class="space-y-3.5">
                            <?php $__currentLoopData = $features; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feature): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex items-start gap-3 text-sm text-slate-300">
                                    <div class="w-5 h-5 rounded-full <?php echo e($isPopular ? 'bg-indigo-500/20 text-indigo-400' : 'bg-emerald-500/10 text-emerald-400'); ?> flex items-center justify-center flex-shrink-0 mt-0.5">
                                        <i class="fa-solid fa-check text-xs"></i>
                                    </div>
                                    <span class="leading-snug"><?php echo e($feature); ?></span>
                                </li>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-right border-collapse">
                    <thead>
                        <tr class="border-b border-slate-800 bg-slate-900/80">
                            ...
                            <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php $isPopular = $plan['is_popular'] ?? false; ?>
                                <th class="py-6 px-6 text-center <?php echo e($isPopular ? 'bg-indigo-950/40 border-x border-indigo-500/30' : ''); ?> w-2/9">
                                    <div class="text-lg md:text-xl font-black text-white mb-1"><?php echo e($plan['name']); ?></div>
                                    <?php if($isPopular): ?>
                                        <span class="inline-block px-2.5 py-0.5 bg-indigo-500 text-white font-bold text-xs rounded-full">الأكثر طلباً ⭐</span>
                                    <?php else: ?>
                                        <span class="text-xs text-slate-400 font-normal block"><?php echo e($plan['slug'] === 'basic' ? 'للمبتدئين' : 'للشركات'); ?></span>
                                    <?php endif; ?>
                                </th>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </tr>
                    ...
                    <tbody class="divide-y divide-slate-800/60">
                        <?php $__currentLoopData = $comparisonCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <!-- Category Header -->
                            <tr class="bg-slate-900/50">
                                <td colspan="4" class="py-4 px-6 text-indigo-400 font-black text-base md:text-lg bg-indigo-950/20 border-y border-indigo-500/20">
                                    <i class="fa-solid fa-layer-group mr-2"></i> <?php echo e($group['category']); ?>

                                </td>
                            </tr>
                            <!-- Category Features -->
                            <?php $__currentLoopData = $group['features']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $feat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-slate-900/40 transition-colors">
                                    ...
                                    <?php $__currentLoopData = ['basic', 'pro', 'enterprise']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $planSlug): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <?php
                                            $val = $feat[$planSlug] ?? false;
                                            $isProCol = ($planSlug === 'pro');
                                        ?>
                                        <td class="py-4 px-6 text-center font-bold text-sm md:text-base <?php echo e($isProCol ? 'bg-indigo-950/20 font-black text-indigo-300 border-x border-indigo-500/20' : 'text-slate-300'); ?>">
                                            <?php if($val === true): ?>
                                            ...
                                            <?php else: ?>
                                                <span class="<?php echo e($isProCol ? 'text-white' : ''); ?>"><?php echo e($val); ?></span>
                                            <?php endif; ?>
                                        </td>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </tr>
                    ...
                    <tfoot>
                        <tr class="bg-slate-900/90 border-t border-slate-800">
                            ...
                            <?php $__currentLoopData = $plans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $plan): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php $isPopular = $plan['is_popular'] ?? false; ?>
                                <td class="py-6 px-6 text-center <?php echo e($isPopular ? 'bg-indigo-950/40 border-x border-indigo-500/30' : ''); ?>">
                                    <a href="<?php echo e(Route::has('register') ? route('register', ['plan' => $plan['slug'] ?? '']) : '#'); ?>" 
                                       class="inline-block py-2.5 px-6 rounded-xl font-bold text-sm transition-all shadow-md <?php echo e($isPopular ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-500/30' : 'bg-slate-800 hover:bg-slate-700 text-slate-200'); ?>">
                                        ابدأ التجربة مجاناً
                                    </a>
                                </td>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <div class="space-y-4">
            <?php $__currentLoopData = $faqs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $faq): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="glass-card rounded-2xl overflow-hidden border border-slate-800 transition-colors hover:border-slate-700">
                    <button 
                        @click="activeFaq === <?php echo e($index); ?> ? activeFaq = null : activeFaq = <?php echo e($index); ?>" 
                        ...
                        <div class="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 flex-shrink-0 transition-transform duration-300" 
                             :class="activeFaq === <?php echo e($index); ?> ? 'rotate-180 bg-indigo-600 text-white' : ''">
                    ...
                    <div x-show="activeFaq === <?php echo e($index); ?>" 
                        ...
                        <p class="pr-10"><?php echo e($faq['answer']); ?></p>
                    </div>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    ...
    <section class="py-16 px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="max-w-6xl mx-auto rounded-3xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 p-8 sm:p-14 text-center relative overflow-hidden shadow-2xl shadow-indigo-600/20">
            ...
            <div class="flex flex-wrap items-center justify-center gap-4 relative z-10">
                <?php if(Route::has('register')): ?>
                    <a href="<?php echo e(route('register')); ?>" class="px-8 py-4 rounded-2xl bg-white text-slate-950 hover:bg-slate-100 font-black text-base shadow-xl transition-all transform hover:-translate-y-1">
    ...
    <footer class="mt-auto border-t border-slate-900 bg-slate-950/80 py-8 px-4 text-center text-slate-500 text-sm z-10">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div>
                &copy; <?php echo e(date('Y')); ?> جميع الحقوق محفوظة لمنصة فاست أوردر.
            </div>
        </div>
    </footer>
...
<?php /**PATH E:\programing\flutter project\fast order\resources\views\platform\pricing.blade.php ENDPATH**/ ?>