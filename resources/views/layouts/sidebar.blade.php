<aside class="hidden sm:block">
    ...
    @endphp
    <div x-data="{ profileOpen: false }" class="fixed inset-y-0 {{ $isRtl ? 'right-0 border-l' : 'left-0 border-r' }} w-64 bg-white shadow-xl z-40 p-5 flex flex-col border-gray-200">
        ...
        <nav class="flex flex-col gap-1" aria-label="{{ __('القائمة') }}">
            <a href="/admin/dashboard" {!! request()->is('admin/dashboard') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/dashboard') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
                <!-- icon: home -->
            ...
            <a href="/admin/orders" {!! request()->is('admin/orders*') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/orders*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
                <!-- icon: orders/list -->
            ...
            <a href="/admin/categories" {!! request()->is('admin/categories*') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/categories*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
                <!-- icon: folders -->
            ...
            <a href="/admin/products" {!! request()->is('admin/products*') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/products*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
                <!-- icon: tag/product -->
            ...
            <a href="/admin/shipping" {!! request()->is('admin/shipping*') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/shipping*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
                <!-- icon: truck/shipping -->
            ...
            <a href="/admin/banners" {!! request()->is('admin/banners*') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/banners*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
                ...
                <span>{{ __('البانرات') }}</span>
            </a>
            <a href="/admin/settings" {!! request()->is('admin/settings*') ? 'aria-current="page"' : '' !!} class="group flex items-center gap-3 px-3 py-2 rounded-lg transition-colors hover:bg-indigo-50 {{ request()->is('admin/settings*') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-700' }}">
        ...
        <div class="mt-auto pt-5">
            ...
            <div class="mb-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider"><i class="fa-solid fa-globe mr-1"></i>{{ __('اللغة') }}</span>
                <div class="flex items-center gap-1 bg-gray-100 p-0.5 rounded-lg">
                    <a href="{{ route('lang.switch', 'ar') }}" class="px-2.5 py-1 text-xs rounded-md transition-all {{ $isRtl ? 'bg-white text-indigo-600 font-bold shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">العربية</a>
                    <a href="{{ route('lang.switch', 'en') }}" class="px-2.5 py-1 text-xs rounded-md transition-all {{ !$isRtl ? 'bg-white text-indigo-600 font-bold shadow-sm' : 'text-gray-600 hover:text-gray-900' }}">English</a>
                </div>
            </div>

            <div class="relative">
                <button @click="profileOpen = !profileOpen" aria-haspopup="true" :aria-expanded="profileOpen.toString()" aria-label="{{ __('قائمة الملف الشخصي') }}" class="w-full flex items-center justify-between px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50">
                    <div class="flex items-center gap-2">
                        @if (file_exists($logoPath))
                            <img src="{{ asset('images/logo.png?v=202604031') }}" alt="{{ __('شعار المتجر') }}" class="h-6 w-6 rounded-full border border-gray-200 object-cover" />
                        @else
                            <img src="https://dummyimage.com/24x24/111827/ffffff&text=K" alt="{{ __('الصورة الشخصية الافتراضية') }}" class="h-6 w-6 rounded-full border border-gray-200 object-cover" />
                        @endif
                        <span class="text-sm text-gray-700">{{ __('الملف الشخصي') }}</span>
                    </div>
                ...
                <div x-show="profileOpen" x-transition @click.outside="profileOpen=false" class="absolute bottom-12 right-3 left-3 bg-white border border-gray-200 rounded-lg shadow-lg py-2 z-50">
                    <a href="/admin/profile" class="block px-3 py-2 text-sm hover:bg-gray-50">{{ __('الملف الشخصي') }}</a>
                    <form method="POST" action="/admin/logout">
                        @csrf
                        <button type="submit" class="w-full text-right px-3 py-2 text-sm text-red-600 hover:bg-red-50">{{ __('تسجيل الخروج') }}</button>
                    </form>
                </div>
            </div>
        </div>
