<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <form method="POST" action="{{ route('categories.update', $category) }}" enctype="multipart/form-data" class="text-right" id="categoryEditForm">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="block mb-1 font-medium">اسم القسم <span class="text-red-500">*</span></label>
                    <input type="text" name="name_ar" value="{{ old('name_ar', $category->name_ar ?? $category->name) }}" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-200" required>
                        ...
                        <div class="flex-1 relative">
                            <select name="main_category" class="w-full border rounded-lg p-2 pr-8 focus:ring-2 focus:ring-indigo-200" required style="-webkit-appearance: none; -moz-appearance: none; appearance: none; background-image: none;">
                                ...
                                @foreach(\App\Models\Category::getMainCategories() as $mainCat)
                                    <option value="{{ $mainCat }}" {{ old('main_category', $category->main_category) == $mainCat ? 'selected' : '' }}>
                                        {{ $mainCat }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-indigo-200" rows="4">{{ old('description', $category->description) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    @if($category->image_path)
                        <div class="mb-2">
                            <img src="{{ asset('storage/'.$category->image_path) }}" alt="صورة القسم الحالية" class="h-24 w-24 object-cover rounded border">
                        </div>
                    @endif
