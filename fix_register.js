...
const oldPassBlock =             <div>
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-1.5">كلمة المرور</label>
                <input id="password" class="arabic-input block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200"
                            type="password"
                            name="password"
                            required 
                            autocomplete="new-password"
                            placeholder="أدخل كلمة المرور" />
                <x-input-error :messages="\\->get('password')" class="mt-1" />
            </div>;

const newPassBlock =             <div x-data="{ show: false }">
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-1.5">كلمة المرور</label>
                <div class="relative">
...
file = file.replace(oldPassBlock, newPassBlock);

const oldPassConfirmBlock =             <div>
                <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 mb-1.5">تأكيد كلمة المرور</label>
                <input id="password_confirmation" class="arabic-input block w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200"
                            type="password"
                            name="password_confirmation" 
                            required 
                            autocomplete="new-password"
                            placeholder="أعد إدخال كلمة المرور" />
                <x-input-error :messages="\\->get('password_confirmation')" class="mt-1" />
            </div>;

const newPassConfirmBlock =             <div x-data="{ show: false }">
                <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 mb-1.5">تأكيد كلمة المرور</label>
                <div class="relative">
...
file = file.replace(oldPassConfirmBlock, newPassConfirmBlock);

fs.writeFileSync('resources/views/auth/register.blade.php', file);
